...
def main():
    ...
    os.chdir(pyPath)
    mdb = Mdb(pathName=pyPath+'/Analysis_uni_3.cae')
    myModel = mdb.Model(name='Analysis_uni_3')
    myAssembly = myModel.rootAssembly
    mappedMeshFlag = 1
    currentMat = myModel.Material(name='matrix')
    currentMat.Density(table=((1100,),))
    currentMat.Elastic(type = ISOTROPIC,temperatureDependency=OFF, table=[(4000,0.35)])
    ...
    myAcis = mdb.openParasolid(pyPath+'/Job_uni_3_Analysis_uni_3_Phase1_uncut.xmt_txt',topology=SOLID)
    if(myAcis.numberOfParts>1):
        inst2=[]
        for j in range(1,myAcis.numberOfParts+1):
            myName = 'Job_uni_3_Analysis_uni_3_Pha~1_'+str(j)
        ...
        matrixInst = myAssembly.InstanceFromBooleanMerge(name='Job_uni_3_Analysis_uni_3_Pha~1',instances=inst2,keepIntersections=OFF,originalInstances=SUPRESS,domain=GEOMETRY)
    else:
        myName='Job_uni_3_Analysis_uni_3_Pha~1'
        myModel.PartFromGeometryFile(myName,myAcis,THREE_D,DEFORMABLE_BODY,scale=0.001)
        instanceName = myName
    ...
    if(myAttributes['numCells']==0 and myAttributes['numWireEdges']>0):
        matrixSet = matrixPart.Set(name='SetJob_uni_3_Analysis_uni_3_Pha~1',edges=matrixPart.edges)
    else:
        matrixSet = matrixPart.Set(name='SetJob_uni_3_Analysis_uni_3_Pha~1',cells=matrixPart.cells)    
    ...
    if(len(inst)>1):
        ...
        matrixInst = inst[0]
        matrixPart = myModel.Part(name='Matrix',objectToCopy=myModel.parts['Job_uni_3_Analysis_uni_3_Pha~1'])
    ...
    partNames['Phase2']=[]
    myAcisIncl = mdb.openParasolid(pyPath+'/Job_uni_3_Analysis_uni_3_Phase2.xmt_txt',topology=SOLID)
    ...
    if(continueFlag==NO):
        mdb.saveAs(pathName=pyPath+'/Analysis_uni_3.cae')
        return 0;
    ...
    region = myModel.parts['matrixCut'].sets['SetJob_uni_3_Analysis_uni_3_Pha~1']
    ...
    myModel.DisplacementBC(name='BC-10',createStepName='Step-1',region=myAssembly.sets['refpoint6'], u3=c*0.03, distributionType=UNIFORM, fieldName='',localCsys=None)
    mdb.Job(name='Job_Analysis_uni_3',model='Analysis_uni_3',type=ANALYSIS, explicitPrecision=SINGLE,nodalOutputPrecision=SINGLE,description='',parallelizationMethodExplicit=DOMAIN,multiprocessingMode=DEFAULT,numDomains=1, userSubroutine='',numCpus=1,scratch='',echoPrint=OFF, modelPrint=OFF,contactPrint=OFF, historyPrint=OFF)
    xMid = sizeCube[0]/2
    yMid = sizeCube[1]/2
    zMid = sizeCube[2]/2
    ...
    mdb.jobs['Job_Analysis_uni_3'].writeInput(consistencyChecking=OFF)
    mdb.saveAs(pyPath+'/Analysis_uni_3.cae')
    print '********************'
