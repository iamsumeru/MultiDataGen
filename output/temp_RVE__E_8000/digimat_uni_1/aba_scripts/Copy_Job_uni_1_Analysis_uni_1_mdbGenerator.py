...
def main():
    ...
    os.chdir(pyPath)
    mdb = Mdb(pathName=pyPath+'/Analysis_uni_1.cae')
    myModel = mdb.Model(name='Analysis_uni_1')
    ...
    myAcis = mdb.openParasolid(pyPath+'/job_uni_1_Analysis_uni_1_Phase1_uncut.xmt_txt',topology=SOLID)
    if(myAcis.numberOfParts>1):
        inst2=[]
        for j in range(1,myAcis.numberOfParts+1):
            myName = 'job_uni_1_Analysis_uni_1_Pha~1_'+str(j)
        ...
        matrixInst = myAssembly.InstanceFromBooleanMerge(name='job_uni_1_Analysis_uni_1_Pha~1',instances=inst2,keepIntersections=OFF,originalInstances=SUPRESS,domain=GEOMETRY)
    else:
        myName='job_uni_1_Analysis_uni_1_Pha~1'
        myModel.PartFromGeometryFile(myName,myAcis,THREE_D,DEFORMABLE_BODY,scale=0.001)
        instanceName = myName
    ...
    if(myAttributes['numCells']==0 and myAttributes['numWireEdges']>0):
        matrixSet = matrixPart.Set(name='Setjob_uni_1_Analysis_uni_1_Pha~1',edges=matrixPart.edges)
    else:
        matrixSet = matrixPart.Set(name='Setjob_uni_1_Analysis_uni_1_Pha~1',cells=matrixPart.cells)    
    ...
    if(len(inst)>1):
        ...
        matrixInst = inst[0]
        matrixPart = myModel.Part(name='Matrix',objectToCopy=myModel.parts['job_uni_1_Analysis_uni_1_Pha~1'])
    ...
    partNames['Phase2']=[]
    myAcisIncl = mdb.openParasolid(pyPath+'/job_uni_1_Analysis_uni_1_Phase2.xmt_txt',topology=SOLID)
    ...
    if(continueFlag==NO):
        mdb.saveAs(pathName=pyPath+'/Analysis_uni_1.cae')
        return 0;
    ...
    region = myModel.parts['matrixCut'].sets['Setjob_uni_1_Analysis_uni_1_Pha~1']
    ...
    mdb.jobs['Job_Analysis_uni_1'].writeInput(consistencyChecking=OFF)
    mdb.saveAs(pyPath+'/Analysis_uni_1.cae')
    print '********************'
