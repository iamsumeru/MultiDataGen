...
def main():
    ...
    os.chdir(pyPath)
    mdb = Mdb(pathName=pyPath+'/Analysis_sh_13.cae')
    myModel = mdb.Model(name='Analysis_sh_13')
    ...
    myAcis = mdb.openParasolid(pyPath+'/Job_sh_13_Analysis_sh_13_Phase1_uncut.xmt_txt',topology=SOLID)
    if(myAcis.numberOfParts>1):
        inst2=[]
        for j in range(1,myAcis.numberOfParts+1):
            myName = 'Job_sh_13_Analysis_sh_13_Pha~1_'+str(j)
        ...
        matrixInst = myAssembly.InstanceFromBooleanMerge(name='Job_sh_13_Analysis_sh_13_Pha~1',instances=inst2,keepIntersections=OFF,originalInstances=SUPRESS,domain=GEOMETRY)
    else:
        myName='Job_sh_13_Analysis_sh_13_Pha~1'
        myModel.PartFromGeometryFile(myName,myAcis,THREE_D,DEFORMABLE_BODY,scale=0.001)
        instanceName = myName
    ...
    if(myAttributes['numCells']==0 and myAttributes['numWireEdges']>0):
        matrixSet = matrixPart.Set(name='SetJob_sh_13_Analysis_sh_13_Pha~1',edges=matrixPart.edges)
    else:
        matrixSet = matrixPart.Set(name='SetJob_sh_13_Analysis_sh_13_Pha~1',cells=matrixPart.cells)    
    ...
    if(len(inst)>1):
        ...
        matrixInst = inst[0]
        matrixPart = myModel.Part(name='Matrix',objectToCopy=myModel.parts['Job_sh_13_Analysis_sh_13_Pha~1'])
    ...
    partNames['Phase2']=[]
    myAcisIncl = mdb.openParasolid(pyPath+'/Job_sh_13_Analysis_sh_13_Phase2.xmt_txt',topology=SOLID)
    ...
    if(continueFlag==NO):
        mdb.saveAs(pathName=pyPath+'/Analysis_sh_13.cae')
        return 0;
    ...
    region = myModel.parts['matrixCut'].sets['SetJob_sh_13_Analysis_sh_13_Pha~1']
    ...
    myModel.DisplacementBC(name='BC-4',createStepName='Step-1',region=myAssembly.sets['refpoint4'], u3=a*0.03, distributionType=UNIFORM, fieldName='',localCsys=None)
    ...
    mdb.jobs['Job_Analysis_sh_13'].writeInput(consistencyChecking=OFF)
    mdb.saveAs(pyPath+'/Analysis_sh_13.cae')
    print '********************'
